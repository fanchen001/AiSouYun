...
public class StatusExpandAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		final ChildStatusEntity entity = (ChildStatusEntity) getChild(groupPosition,childPosition);
		if (convertView != null) {
		...
		convertView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if(!TextUtils.isEmpty(entity.getCompleteTime())){
					 if(NetworkStateUtil.isNetWorkAvailable(mContext)){
						 ((MainActivity)mContext).setTitle("搜索："+entity.getCompleteTime());
						 ((MainActivity)mContext).changeFragment(new SearchFragment(),false,"search","word",entity.getCompleteTime());
					 }else{
						 Toast.makeText(mContext, "当前网络不可用", 0).show();
					 }
				 }else{
					 Toast.makeText(mContext, "请输入要搜索的资源名称", 0).show();
				 }
			}
		});
